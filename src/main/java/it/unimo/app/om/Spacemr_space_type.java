...
//-
public class Spacemr_space_type extends BaseObjectModel {
   //-
   //-
   @Autowired
   ...
   public int insert(JSONObject content) throws Exception {
      ...
      String qs = ""
         + "insert into spacemr_space_type ("
   ...
   //-
   //-
   public JSONObject get(int spacemr_space_type_id) throws Exception {
      return(get(spacemr_space_type_id, getColumns()));
   }
   
   //-
   public JSONObject get(int spacemr_space_type_id, Vector<String> columns) throws Exception {
      AppMapperJson mapper = new AppMapperJson();
      String fieldsToGet = "spacemr_space_type_id, " 
      ...
      jdbcTemplate.query("select "
                         ...
                         + " where spacemr_space_type_id = ?"
                         // + "   and spacemr_space_type_super.spacemr_space_type_super_id = spacemr_space_type.spacemr_space_type_super_id"
                         , new Object[] { Integer.valueOf(spacemr_space_type_id) }
                         , mapper);
   ...
   //-
   public JSONObject getLog(int spacemr_space_type_id) throws Exception {
      AppMapperJson mapper = new AppMapperJson();
      jdbcTemplate.query("select * from spacemr_space_type where spacemr_space_type_id = ?"
                         , new Object[] { Integer.valueOf(spacemr_space_type_id) }
                         , mapper);
   ...
   public void update(JSONObject content) throws Exception {
      ...
      np.addValue("spacemr_space_type_id", content.getInt("spacemr_space_type_id"));
      String qs = 
         "update spacemr_space_type set "
         + getSqlStringForUpdate(getColumnsWrite())
         + " where spacemr_space_type_id = :spacemr_space_type_id"
         ;
      namedParameterJdbcTemplate.update(qs, np );
   }
   
   //-
   public void delete(int spacemr_space_type_id) throws Exception {
      app_log.deleteLogs(this, spacemr_space_type_id);
      MapSqlParameterSource np = new MapSqlParameterSource();
      np.addValue("spacemr_space_type_id", spacemr_space_type_id);
      namedParameterJdbcTemplate
         .update(
                 "delete from spacemr_space_type  "
                 + " where spacemr_space_type_id = :spacemr_space_type_id"
                 , np
                 );
   }
   //-
   public JSONArray getAllSpaceTypes() throws Exception {
      AppMapperJson mapper = new AppMapperJson();
      jdbcTemplate.query("select spacemr_space_type_id, name from spacemr_space_type order by name", mapper);
   ...
   static  {
      // computedColumnsMap.put("spacemr_space_type_super_name", "spacemr_space_type_super.name");
      // computedColumnsMap
      //    .put("user_first_name"
      //         , ""
      //         + "(select first_name from app_user where app_user.user_name = spacemr_space_type.user_name)"
      //         );
   }
}
